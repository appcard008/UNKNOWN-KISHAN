...
if (typeof SECRETS === 'undefined') {
    console.error("Config not found! SECRETS is undefined.");
    alert("Critical: config.js not found or SECRETS not defined!");
}

let database;
try {
    if (typeof SECRETS !== 'undefined' && SECRETS.FIREBASE_CONFIG) {
...
async function hashPassword(string) {
    if (!window.crypto || !window.crypto.subtle) {
        console.warn("Cryptography API not available. Using plain text fallback (Check HTTPS).");
        return string;
    }
...
function startAdminStatusMonitor(username, enteredPassword = null) {
    ...
    if (!database) {
        ...
        if (errorMsg) {
            errorMsg.innerText = "Database connection not initialized.";
    ...
    adminStatusRef.on('value', async snapshot => {
        ...
        const errorMsg = document.getElementById('login-error');
        if (errorMsg) {
            errorMsg.innerText = "Permission denied or Network error.";
            errorMsg.classList.remove('hidden');
        }
    });
}
